(() => {
    ...
    // --- Fun little break: the profile-menu mini-game ---
    function openGameModal() {
        document.getElementById('userDropdown').classList.add('hidden');
        document.getElementById('gameModal').classList.remove('hidden');
        if (window.HillelRunner) window.HillelRunner.start();
    }

    function closeGameModal() {
        document.getElementById('gameModal').classList.add('hidden');
        if (window.HillelRunner) window.HillelRunner.stop();
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        document.getElementById('closeGameModalBtn2').addEventListener('click', closeGameModal);
        document.getElementById('gameRestartBtn').addEventListener('click', () => {
            if (window.HillelRunner) {
                window.HillelRunner.stop();
                window.HillelRunner.start();
            }
        });
