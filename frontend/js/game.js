...
(function () {
    ...
    // A full jump (up + down) takes about GRAVITY/JUMP_VELOCITY*2 =~ 0.65s no
    // matter how fast the ground is scrolling. MIN_SPAWN_GAP is chosen well
    // above that fixed number so an obstacle is always jumpable in time, even
    // late in a run - the earlier version let speed and gap shrink together,
    // which made it feel fast right away and become genuinely impossible
    // (not just hard) after a few seconds. This version stays a slow, classic
    // time-waster: gentle start, and it takes a while to get properly fast.
    const BASE_SPEED = 165;      // px / s
    const MAX_SPEED = 260;       // px / s - deliberately a mild ramp, not an escalating challenge
    const SPEED_RAMP_PER_SECOND = 2; // px/s of extra speed per second survived
    const MIN_SPAWN_GAP = 1.15;  // seconds - always well above the ~0.65s jump time
    const MAX_SPAWN_GAP = 2.3;   // seconds
    // Collision uses a shrunken box relative to what's actually drawn, on both
    // the player and the obstacles - this is the classic "generous hitbox"
    // trick casual games use so near-misses (which look fine to the eye) don't
    // feel unfair, since this game is meant to be a relaxed time-waster rather
    // than a precision test.
    const OBSTACLE_HITBOX_INSET_X = 0.18; // fraction of width trimmed off each side
    const OBSTACLE_HITBOX_TOP_FRACTION = 0.35; // ignore this fraction off the top
    ...
    function resetGame() {
        ...
        obstacles = [];
        nextSpawnIn = 1.6; // a calm moment before the first obstacle shows up
        elapsed = 0;
        score = 0;
        started = false;
        over = false;
    ...
    function currentSpeed() {
        return Math.min(MAX_SPEED, BASE_SPEED + elapsed * SPEED_RAMP_PER_SECOND);
    }
    ...
    function update(dt) {
        ...
        // Collision - generous hitboxes on both sides so near-misses (which
        // look fine to the eye) don't feel unfair. See the constants above.
        const margin = 5;
        const playerBox = { x: PLAYER_X + margin, y: player.y + margin, width: PLAYER_WIDTH - margin * 2, height: PLAYER_HEIGHT - margin * 2 };
        for (const obstacle of obstacles) {
            const inset = obstacle.width * OBSTACLE_HITBOX_INSET_X;
            const topCut = obstacle.height * OBSTACLE_HITBOX_TOP_FRACTION;
            const obstacleBox = {
                x: obstacle.x + inset,
                y: obstacle.y + topCut,
                width: obstacle.width - inset * 2,
                height: obstacle.height - topCut,
            };
