...
class FirestoreRestClient {
  ...
  async getDocument(collectionPath, id) {
    const headers = await this._authHeaders();
    const res = await fetch(`${this.documentsBaseUrl}/${collectionPath}/${encodeURIComponent(id)}`, { headers });
    if (res.status === 404) return null;
    if (!res.ok) {
      throw new Error(`Firestore get failed for ${collectionPath}/${id} (${res.status})`);
    }
  ...
  async updateDocument(collectionPath, id, patch) {
    ...
    const mask = Object.keys(patch)
      ...
      .join('&');
    const res = await fetch(`${this.documentsBaseUrl}/${collectionPath}/${encodeURIComponent(id)}?${mask}`, {
      method: 'PATCH',
      headers,
  ...
  async deleteDocument(collectionPath, id) {
    const headers = await this._authHeaders();
    const res = await fetch(`${this.documentsBaseUrl}/${collectionPath}/${encodeURIComponent(id)}`, {
      method: 'DELETE',
      headers,
    });
