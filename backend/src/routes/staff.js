...
function createStaffRouter({ superAdminUsername, defaultPassword }) {
  ...
  router.post('/', requireAdmin, async (req, res) => {
    ...
    if (!(await credentials.hasCredentials(username))) {
      await credentials.setPassword(username, defaultPassword);
    }
  ...
  router.delete('/:id', requireAdmin, async (req, res) => {
    ...
    await credentials.removeCredentials(target.username);
    res.json({ ok: true });
  });
  ...
  router.post('/:id/reset-password', requireAuth, async (req, res) => {
    ...
    await credentials.setPassword(target.username, newPassword);
    res.json({ ok: true });
  });
